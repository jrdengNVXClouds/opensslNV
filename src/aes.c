...
SEXP R_aes_any(SEXP x, SEXP key, SEXP iv, SEXP encrypt, SEXP cipher) {
  ...
  int tag_len = 16;//novo vivo
  int outlen = LENGTH(x) + blocksize - remainder + tag_len;//novo vivo
  ...
  cur += tmp;
    ...
    /* hack for GCM tag NovoVivo */
    if(EVP_CIPHER_mode(cph) == EVP_CIPH_GCM_MODE){
        bail(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, tag_len, cur));
        cur += tag_len;
    }
    /* hack for GCM tag NovoVivo */
#endif
